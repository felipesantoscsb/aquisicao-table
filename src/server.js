...
app.get('/oferta-essential-isa',     (req, res) => res.sendFile(path.join(ofertas, 'table-essential-isa.html')));
app.get('/oferta-essential-pr-isa',  (req, res) => res.sendFile(path.join(ofertas, 'table-essential-pr-isa.html')));
app.get('/oferta-premium-isa',       (req, res) => res.sendFile(path.join(ofertas, 'table-premium-isa.html')));
app.get('/oferta-premium-pr-isa',    (req, res) => res.sendFile(path.join(ofertas, 'table-premium-pr-isa.html')));
app.get('/oferta-elite-isa',         (req, res) => res.sendFile(path.join(ofertas, 'table-elite-isa.html')));
app.get('/oferta-elite-pr-isa',      (req, res) => res.sendFile(path.join(ofertas, 'table-elite-pr-isa.html')));
...
app.get('/proximo-passo', (req, res) => res.sendFile(path.join(funil, 'pagina_vendas_rmkt.html')));
app.get('/rmkt',       (req, res) => res.redirect(301, '/proximo-passo' + (req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '')));
...
const MAPA_LIA_EVENTS = new Set([
  ...
  'evelyn_audio_played',
  'quiz_back_clicked', 'draft_resume_shown', 'draft_resumed', 'draft_restart_clicked',
...
app.post('/api/webhooks/cakto', async (req, res) => {
  ...
  try {
    ...
    if (aprovado) {
      ...
      // Isolamento: vendas do funil TikTok NÃO vão pro Meta (têm webhook próprio,
...
// ─── Webhook Cakto → TikTok (CompletePayment) ────────────────────────────────
// Endpoint DEDICADO ao funil TikTok. Crie um webhook novo na Cakto apontando pra
// cá. Recebe as vendas e só dispara CompletePayment no TikTok quando a venda veio
// do canal TikTok (lead:{sck}.channel === 'tiktok'). Isolado do Meta.
// VALIDAR com Cakto (modo teste) + TikTok Test Events antes de confiar.
app.post('/api/webhooks/cakto-tiktok', async (req, res) => {
  res.sendStatus(200);
  try {
    const body = req.body || {};
    const expected = process.env.CAKTO_WEBHOOK_SECRET;
    if (expected && body.secret !== expected) { console.warn('[Cakto-TT] secret inválido — ignorado.'); return; }
    ...
    const srcLeadId = extractCaktoJoinKey(d);
    if (!srcLeadId) { console.warn('[Cakto-TT] venda sem join key (sck) — não atribui.'); return; }

    // Só age se o lead veio do funil TikTok
    let lead = null;
    try { const lr = await getRedis().get('lead:' + srcLeadId); if (lr) lead = JSON.parse(lr); } catch {}
    if (!lead || lead.channel !== 'tiktok') {
      console.log('[Cakto-TT] venda não-TikTok (' + (lead?.channel || 'sem lead') + ') — ignorada.');
      return;
    }

    const TOKEN = process.env.TIKTOK_CAPI_TOKEN;
    const PIXEL = process.env.TIKTOK_PIXEL_ID || 'D9KG3FBC77U751E5R0AG';
    if (!TOKEN) { console.error('[Cakto-TT] TIKTOK_CAPI_TOKEN ausente — evento não enviado.'); return; }

    const value = typeof d.amount === 'number' ? d.amount : Number(d.amount) || null;
    const email = d.customer?.email || lead.email || null;
    const phone = normalizePhone(d.customer?.phone || lead.phone || '');

    const user = {};
    if (email)            user.email = sha256(String(email).toLowerCase().trim());
    if (phone)            user.phone = sha256('+' + phone);
    if (lead.external_id) user.external_id = lead.external_id;
    ...
    const payload = {
      ...
      data: [{
        ...
        event_time: Math.floor(Date.now() / 1000),
        event_id:   String(id),   // dedup por transação (retries do webhook)
        user,
        page:       { url: 'https://www.evelynliu.com.br/raiz-tiktok' },
        properties: {
          value:    value != null ? value : undefined,
          currency: 'BRL',
          contents: [{ content_id: 'protocolo-raiz', content_type: 'product', content_name: d.product?.name || 'Protocolo Raiz' }],
        },
      }],
    };
    ...
    const j = await r.json();
    if (j.code !== 0) console.error('[Cakto-TT] Erro TikTok:', JSON.stringify(j));
...
// concluído em 21/07/2026, quando o remarketing ganhou link próprio (antes
// dividia o da bio) e a compra passou a ser atribuível por canal:
//   primário (quiz)      → O3EB65FBD   /raiz
//   dossiê               → O9E1D1B1F   dossiês disparados pelo sdr-table
//   SDR conversa         → OBEF2D02E   link que o agente manda no WhatsApp
//   remarketing          → O0704BE5A   /proximo-passo
//   bio (via /links)     → O972A923C   /protocolo-raiz
// OBEF2D02E ficou no SDR de propósito: as vendas históricas nesse link vieram
// de conversa com o agente, então o histórico segue coerente com a atribuição.
...
//   OFFER_IDS_PRIMARIO / OFFER_IDS_DOSSIE / OFFER_IDS_SDR / OFFER_IDS_RMKT /
...
function canalDaOferta(offerId) {
  ...
  if (offerIdsDoEnv('OFFER_IDS_SDR').includes(id))      return 'sdr_conversa';
